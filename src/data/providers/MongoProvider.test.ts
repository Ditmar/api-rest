...
describe('MongoProvider Integration Tests', () => {
    ...
    beforeAll(async () => {
        ...
        console.log(`Test MongoDB connected to: ${uri}`);
    ...
    it('get() should retrieve all documents', async () => {
        ...
        expect(results.find(r => r.name === item1.name && r.count === item1.count)).toBeDefined();
        expect(results.find(r => r.name === item2.name && r.count === item2.count)).toBeDefined();
    });
