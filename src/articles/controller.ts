...
const articlesController = (dataCollection: BaseCollection) => {
  const get = async (req: Request, res: Response) => {
    try {
      const data = await dataCollection.get();
      res.status(HttpStatus.OK).json({ message: "Hello from articles controller", data });
    } catch (error: unknown) {
  ...
  return { get, getById, post, put, deleteArticle };
};
